...
- 所有 Markdown / MDX 内容（包括首页、Overview 和目录页）必须在 frontmatter 中记录 `createdAt`、`updatedAt`、`timeSource`；公众号文章 `src/content/docs/baiyun/` 下除 `index` 外的内容豁免，保留原有发表信息。
- `createdAt` 为建立时间，`updatedAt` 为最近修改时间，均使用带时区、精确到秒的 ISO 8601 字符串，例如 `"2026-09-09T14:30:00+08:00"`。修改时间不得早于建立时间。
- 优先采用原笔记明确记录且可信的创建、修改时间，标记 `timeSource: source`；全新站内内容使用实际创建时间，标记 `timeSource: manual`。不得把复制时间、文件系统 ctime、构建或部署时间冒充内容建立时间。
- 已有内容缺少原始时间时，采用 Git 跟随重命名历史中的最早、最近提交时间，标记 `timeSource: git`。此值仅代表本站可追溯记录，不声称是原文实际创作时间；页面悬停提示须说明初始时间来源。本次统一补录时间字段不视为正文修改。
- 后续修改正文、标题、公式、代码、图片或内容链接时，必须同步更新 `updatedAt` 为实际修改时间；`createdAt` 保持不变。`timeSource` 记录初始时间来源，后续正常修改不覆盖该来源。
- 仅修改全站样式、组件、导航配置、时间字段格式或重新构建时，不批量刷新文章时间。取得更可靠的原笔记时间后，可以校正字段与来源，但须说明依据。
- 时间由统一组件显示在标题区域，首页显示在 Hero 后；顺序为建立时间、最近修改时间。使用时钟图标、低对比度文字，继承站点字体；固定按 `Asia/Shanghai` 显示为 `YYYY年M月D日 HH:mm:ss`，小屏自动换行，并提供可访问名称与机器可读的 `time datetime`。
- 不在原笔记正文插入时间段落，不改动正文结构。新增或更新内容后运行 `npm run check:content-dates`、`npm run check:note-format`，并使用 Node 22 完整构建；构建会强制校验时间字段。

- 深色与浅色模式必须同时验收；时间文字使用站点主题变量，图标继承 currentColor，不得写死仅适用于深色的灰色。时间区域沿用 --font-prose（Maple Mono + 霞鹜文楷）。
